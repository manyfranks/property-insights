...
1. **Add to wizard — classified by regulatory interaction, not lumped as "unlicensed disclosures" (rev 4, superseded by [IMPLEMENTATION-BLUEPRINT.md](IMPLEMENTATION-BLUEPRINT.md) §3):** neutral `PROPERTY_FACT` items (construction type; heating type/fuel; wood stove + WETT status; wiring type; panel amperage; plumbing material (Poly-B!); oil tank presence/age; vacancy day-count; home-based business; mortgage/lienholder) are collectable with reviewed copy/consent; `INSURANCE_HISTORY` items (prior insurer + claims detail (type/date, not just count); DOB + credit consent where permitted) require an approved flow; **coverage selections (dwelling limit, deductible, liability, sewer backup / overland water / earthquake endorsements) are `COVERAGE_CHOICE` — licensed-flow functionality, not pre-licence wizard fields.** Questions are added when a contracted market's field dictionary requires them (Phase B1), not speculatively.
   *Design note: don't front-load — ask the systems block conditionally (old homes only) and let triage decide when detail is needed.*
